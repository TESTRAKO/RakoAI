...
def draw_win(cars, road, world, GEN):     
    ...
    for car in cars:
        car.draw(world)
    text = STAT_FONT.render("Car Score : "+str(int(world.getScore())), 1, YELLOW)
    world.win.blit(text, (world.win_width-text.get_width() - 10, 10))
    text = STAT_FONT.render("Gen: "+str(GEN), 1, YELLOW)
    world.win.blit(text, (world.win_width-text.get_width() - 10, 50))
    text = RAKO_FONT.render("RAKWAN ALI 2025", 1, WHITE)
    world.win.blit(text, (world.win_width-text.get_width() - 10, 650))
